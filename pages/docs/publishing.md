...
> **Push from CI directly?** Since 0.9.0, plugin authors can publish releases from CI with a scoped publisher token instead of relying on the webhook event. See [CI-driven push](authors/publish-push.md) for the workflow.

## What gets indexed

A release is the combination of:

1. A manifest file (`tabularium.yaml`, `tabularium.json`, `.tabularium`, …) at the root of your repository — see [Manifest](manifest.md)
...
If you distribute large binaries and want them in the integrity payload, ask the registry operator to raise the budget. See [Security](admin/security.md).
