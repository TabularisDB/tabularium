...
export const ManifestSchema = Type.Object(
  {
    id: Type.Optional(
      Type.String({
        ...
        pattern: '^[a-z][a-z0-9-]*$',
        description:
          'Stable plugin identifier and URL slug. Pinned at first submit. For legacy manifests without id, name supplies the identifier.',
      }),
    ),
    name: Type.String({
      minLength: 1,
      ...
      pattern: '\\S',
      description:
        'Human-readable display name (e.g. "SQLite JDBC"). Without id, this must remain a lowercase slug (1–64 characters) for backwards compatibility.',
    }),
    version: Type.String({
      minLength: 1,
      maxLength: 40,
  ...
  },
  {
    allOf: [
      {
        if: { required: ['id'] },
        else: { properties: { name: { pattern: '^[a-z][a-z0-9-]*$', maxLength: 64 } } },
      },
    ],
  },
)
