...
| `/api/auth/me/tokens` | cookie / JWT | Create / list / revoke publisher tokens — see [CI-driven push](/authors/publish-push/) |
...
- Response always includes:
  ...
  - `verified` / `verifiedAt` — admin audit signal (see [Verification](/admin/plugins/#Verification)). `verifiedBy` stays admin-only.
...
Every mutation writes an audit-log entry. See [Kinds](/admin/kinds/) for the full reference.
...
Pure JS/TS library — no Bun-specifics, runs in Node ≥ 19 / Deno / browser. Exports `parseManifest`, `validateManifest`, `verifyAssetHash`, `verifyRegistrySignature`. See [Consuming](/consuming/) for verification recipes.
