...
export default new Elysia()
  ...
  .get('/', async ({ params, query, set }) => {
    ...
    detail: { tags: ['Admin'], summary: 'Get a markdown page (raw)', operationId: 'adminGetPage', security: [{ bearerAuth: [] }, { cookieAuth: [] }] },
    params: t.Object({ slug: t.String() }),
    query: t.Object({ locale: t.Optional(localeSchema) }),
    response: { 200: pageSchema, 404: t.Object({ error: t.String() }) },
  })
  .get('/translations', async ({ params }) => {
    const rows = await db.select({
      locale: markdownPages.locale,
      title: markdownPages.title,
      updatedAt: markdownPages.updatedAt,
    }).from(markdownPages).where(eq(markdownPages.slug, params.slug))
    return {
    ...
    detail: { tags: ['Admin'], summary: 'List existing translations for a page', operationId: 'adminListPageTranslations', security: [{ bearerAuth: [] }, { cookieAuth: [] }] },
    params: t.Object({ slug: t.String() }),
    response: { 200: t.Object({ translations: t.Array(translationSummarySchema) }) },
  })
  .patch('/', async ({ params, query, body, set, admin, request }) => {
    const locale = resolveLocale(query.locale)
    ...
    detail: { tags: ['Admin'], summary: 'Update a markdown page (creates the translation row if absent)', operationId: 'updatePage', security: [{ bearerAuth: [] }, { cookieAuth: [] }] },
    params: t.Object({ slug: t.String() }),
    query: t.Object({ locale: t.Optional(localeSchema) }),
    body: t.Object({
  ...
  .delete('/', async ({ params, query, set, admin, request }) => {
    ...
    detail: { tags: ['Admin'], summary: 'Delete a markdown page (all locales) or one translation when `locale` is given', operationId: 'deletePage', security: [{ bearerAuth: [] }, { cookieAuth: [] }] },
    params: t.Object({ slug: t.String() }),
    query: t.Object({ locale: t.Optional(localeSchema) }),
  })
