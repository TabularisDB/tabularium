...
type ParsedPage = { slug: string } & PageFrontmatter & { content: string }

function parseFile(slug: string, raw: string): ParsedPage {
  const match = raw.match(/^---\r?\n([\s\S]*?)\r?\n---\r?\n([\s\S]*)$/)
  if (!match) throw new Error(`${slug}.md: missing or malformed frontmatter`)
  const fm = parseYaml(match[1]) as Partial<PageFrontmatter>
  if (typeof fm.path !== 'string' || typeof fm.title !== 'string') {
    throw new Error(`${slug}.md: frontmatter must include path + title`)
  }
  return {
    slug,
    path: fm.path,
