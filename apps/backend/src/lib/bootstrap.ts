...
export async function initBootstrap(): Promise<void> {
  const plaintext = process.env.BOOTSTRAP_PASSWORD ?? generatePassword()
  const passwordHash = await hashPassword(plaintext)
  state = { email: BOOTSTRAP_EMAIL, passwordHash }
  printBanner(plaintext)
}
...
function printBanner(password: string): void {
  ...
  console.log(`    ${url}`)
  console.log('')
  console.log('  Sign in with these temporary credentials:')
  console.log(`    Email:    ${BOOTSTRAP_EMAIL}`)
  console.log(`    Password: ${password}`)
  console.log('')
  if (!process.env.BOOTSTRAP_PASSWORD) {
    console.log('  (Password regenerated each boot. Set BOOTSTRAP_PASSWORD to pin it.)')
  } else {
    console.log('  (Password sourced from BOOTSTRAP_PASSWORD env var.)')
  }
  console.log(bar)
  console.log('')
}
